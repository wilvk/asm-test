...
([docs/internal/archive/gui/05-loom-day-one.md](../../docs/internal/archive/gui/05-loom-day-one.md)),
...
| `loom-truncated.asmtrace` | the D7 low-fidelity fixture — the same `df_chain` recorded with a **four-record** operand buffer, so the producer flips `truncated` and the footer declares it. Lanes are missing because their records were dropped; the torn edge is what says so |
...
fidelity in this format is a set of *fields*, and a field nothing ever reads is
...
because it carries a fidelity loss — the last pass's operand buffer filled, so its
