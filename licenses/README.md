...
| `ImGuiNotify-LICENSE.txt` | ImGuiNotify (non-modal toasts for live-session events; also supplies the bundled `IconsFontAwesome6.h` macro header) — **bundled**: `ImGuiNotify.hpp` (header-only) compiled into the desktop binaries (gui 16-T1) | Dev commit d00e45f ([fetch-imguinotify.sh](../scripts/fetch-imguinotify.sh)) | MIT |
| `FontAwesome-LICENSE.txt` | Font Awesome Free `fa-solid-900.ttf` (ImGuiNotify's default type icons — a minimal FA range merged into the desktop font) — **bundled**: TTF loaded at runtime by the desktop binaries (gui 16-T1) | 6.x (vendored with ImGuiNotify Dev d00e45f) ([fetch-imguinotify.sh](../scripts/fetch-imguinotify.sh)) | CC-BY-4.0 (icons) / OFL-1.1 (font) |
...
| `imgui-node-editor-LICENSE.txt` | imgui-node-editor (graph pan/zoom canvas) — **bundled**: `imgui_canvas.cpp` (slice-explorer pan/zoom, gui 15-T2) plus the full node editor's `imgui_node_editor.cpp` + `imgui_node_editor_api.cpp` and its bundled `crude_json.cpp` settings serialiser (topo/tree/hot-edge graph nav, gui 15-T3) compiled into the desktop binaries — one fetch, one pin. `crude_json` is vendored inside this repo under the same LICENSE | master 021aa0ea ([fetch-nodeeditor.sh](../scripts/fetch-nodeeditor.sh)) | MIT |
