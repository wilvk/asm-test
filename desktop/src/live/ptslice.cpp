...
PtSliceInput ptslice_input_from(const Recording &r, long tid) {
    ...
    in.base = v->base;
    in.code = v->bytes;
    in.when = v->when; // 37 T4: the exact version's own timestamp
    return in;
}
...
PtSliceResult ptslice_run(const PtSliceInput &in) {
    ...
    for (size_t i = 0; i < vt->steps_len; i++) {
        ...
        // 37 T1: the region base off is relative to — the SAME `in.base` a
        // recorded df_step states as rbase, so the live pane and a replay of the
        // same capture resolve the span identically (0 when no codeimage pinned
        // it). 37 T4: `in.when` rides beside it for the same reason — the whole
        // replay ran against ONE fixed version, so every step shares its `when`.
        out.df.insn_rbase.push_back(in.base);
        out.df.insn_when.push_back(in.when);
