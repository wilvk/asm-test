/*
 * libasmspy.h — THE PUBLIC HEADER of libasmspy, the asm-test out-of-process
 * tracer engine, packaged as a linkable library.
 *
 * libasmspy is the tracer half of asmspy: /proc enumeration, an ELF
 * .symtab/.dynsym + JIT (perf-map / jitdump) function resolver, and the ptrace
 * engines behind every asmspy view (syscall log, instruction stream, region
 * sampler, data flow, call graph, call tree, process topology, the AMD IBS
 * hot-edge sampler, hardware data watchpoints). Two translation units implement
 * it — cli/asmspy_proc.c (the resolvers) and cli/asmspy_engine.c (the engines) —
 * and this is the ONLY header a consumer needs:
 *
 *     #include "libasmspy.h"        // -> build/libasmspy.a  (or libasmspy.so)
 *
 * It exists so consumers link the TESTED engine instead of re-declaring or
 * re-implementing it. The `asmspy` binary is the first consumer (its ncurses TUI
 * and headless subcommands live in cli/asmspy.c behind cli/asmspy.h, which
 * includes this file); `asmspy --serve` is the second, and the desktop GUI
 * reaches the engines ONLY through that serve subprocess — it never links this
 * library (docs/internal/gui/README.md, D9).
 *
 * The unit-tested, dependency-free view-model headers ship alongside it as part
 * of the same public surface: asmspy_logview.h (scrollback viewport math),
 * asmspy_treefilter.h (--tree depth/focus/module), asmspy_dataview.h (data-flow
 * annotation + slice styling) and asmspy_autoregion.h (the --auto entry-edge
 * ranking). Each is header-only and self-contained.
 *
 * LINUX ONLY. The engines are built on ptrace(2), process_vm_readv(2),
 * personality(2), /proc, <sys/user.h> and the glibc extension
 * pthread_timedjoin_np, on x86-64 and AArch64 (the register/step/detach seam is
 * cli/asmspy_arch.h). There is no macOS/BSD/Windows body and no 32-bit body; a
 * 32-bit TRACEE is refused at attach rather than decoded wrong
 * (ASMSPY_ETRACEE_I386).
 *
 ...
 * below therefore run start-to-finish on ONE thread; a front-end runs them on a
 * dedicated tracer thread and never touches ptrace from its UI thread. That is
 * also why every engine takes an `atomic_bool *stop` rather than exposing a
 * cancel call: the owner sets the flag and pthread_kill(SIGALRM)s the tracer
 * thread to unblock a pending waitpid, and the engine returns through its own
 * two-phase detach. Bypassing that teardown is never correct.
 */
#ifndef LIBASMSPY_H
#define LIBASMSPY_H
