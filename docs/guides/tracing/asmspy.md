...
families, `ioctl` requests and `fcntl` commands, and
...
- **Argument decoding is a curated subset.** Every syscall is *named* with
  ...
  paths, buffers) — but struct *contents* (`stat` buffers), `futex` op decoding,
  and `execve` vectors are not decoded, and the undescribed remainder prints
  three raw words + `...`. For exhaustive syscall tracing use `strace`; for
