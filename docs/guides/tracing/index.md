...
| [Native runtime tracing](native-tracing.md) | DynamoRIO (software DBI) + single-step (`EFLAGS.TF`) | native blocks + instructions, in-process | DynamoRIO: Linux x86-64 · macOS x86-64; single-step: x86-64 Linux **or macOS** |
...
| [asmspy CLI](asmspy.md) | out-of-process ptrace (attach) + statistical sampling (IBS-Op, sw-clock, or a perf-free ptrace sampler) | interactive TUI + headless: a running process's live syscalls-with-data, a function's live assembly (heat counts), whole-process call graph (JSON/DOT export) / call tree / instruction stream, the process/thread topology, statistical hot edges sampled out of band (safe on a live JIT), a function's value dataflow (`--dataflow`, with the opt-in recorded streams), hardware data watchpoints (`--watch`), an attach-free process snapshot (`--info`), and the `--serve` capture host the desktop GUI drives; processes sortable by pid / CPU activity / string-scan density | x86-64 + AArch64 Linux (IBS hot-edge sampling: AMD Zen x86-64 only; dataflow: x86-64) |
| [asmspy metrics reference](asmspy-metrics.md) | — | every metric asmspy emits, per view and per output channel (human / `--json` / `.asmtrace`), with its values and the per-architecture observability matrix | reference for the above |
