...
**A second follow-on shipped as `a21a5a95`: the sweep can LEAD with the picker.** The sweep as shipped refused to start without a hand-named region — which closed the door on the mode the patch bay offers first. `SM_AUTO` is the data-flow engine with a sampler in front of it, so an `auto` capture records `codeimage` + `df_step` and nothing else (measured in the cli image with perf available: `codeimage 1, df_step 10, df_edge 9, regstate 10`); the invocation stack needs a `coverage` block set and the ribbon needs `call`, so both are empty **by construction**, and the one control that would fill them was disabled. A sweep now has two shapes: with a region named it is unchanged, and with none it runs `auto -> tree -> trace`, the `trace` leg inheriting the **base+len** the `pick` event carried (not the func name — a name is re-resolved against the symbol table by the host, and a duplicated static symbol resolves to a different function than the one the sampler watched). Every leg needing a region runs after the auto leg, and `test_budget` pins that ORDERING rather than the literal list. An auto leg that picks nothing stops the sweep before the scoped leg with the host's own skip reason — the legs needing no region have already run, which is why `tree` sits in the middle. Measured against `build/auto_victim` with the exact legs and the exact picked region the poll hands on: `codeimage 3, call 400, coverage 400, trace 4000, df_step 10` — **address plane, module excursion ribbon and invocation stack from one recording**. The lane prism stayed empty there and honestly so: that victim's picked routine writes no vector registers.
