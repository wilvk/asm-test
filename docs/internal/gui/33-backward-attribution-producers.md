...
1. **Blame cones.** `blame` is a reserved kind claimed by
   ...
   ([asmtrace-schema.md:323](asmtrace-schema.md#L323)). Doc 09 shipped the
...
2. **Slice-diff / two-recording state-diff.** The slice explorer "explicitly
   ...
   architectural state delta" ([asmtrace-schema.md:322](asmtrace-schema.md#L322)).
...
- **Reserved kinds** — `blame` and `statediff`
  ([asmtrace-schema.md:322-323](asmtrace-schema.md#L322)); adding either is a new
...
**Steps.**
1. Define `blame` fields in [asmtrace-schema.md](asmtrace-schema.md): the queried
