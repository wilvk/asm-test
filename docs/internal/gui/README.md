...
| [32-per-guest-value-producer.md](32-per-guest-value-producer.md) | R5 | arch-parameterize `dataflow_emu.c` (arm64 first) | 3 | independent axis; demand-gated | ◐ 2/3 (T1 `df_guest` vtable, byte-identical x86-64; T2 arm64 value fabric + golden `arm64-df-chain`; **T3 Author-mode arm64 run DONE** — the door dispatches arm64 through the value-fabric producer with an honest, distinct result shape + a materialised `trace`/`df_step`/`df_edge` recording + the accurate gate-flip label, see brief status; **T2's `regstate`/Scrubber sub-item still open**, tracked separately) | claude · T2 regstate/Scrubber ring · 2026-07-28 |
