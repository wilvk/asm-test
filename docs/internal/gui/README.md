...
| [30-resume-from-state-and-reweave.md](30-resume-from-state-and-reweave.md) | R3 | route the value producer through `emu_snapshot`/`emu_restore` | 4 | 05 (forks); 28 T1 (for T4) | ◐ 2/4 (T1 `emu_t`-hosted producer byte-identical to standalone via the shared seed/capture seam on R5's `df_guest`; T2 checkpoint/resume tail-identity + edit-at-K Reweave over `emu_snapshot`/`emu_restore`; `test_reweave`; `make_pair` goldens regenerated for the R1 window churn; `6d16cac`) | *free* (T3 Loom fork-from-K + T4 Scrubber synth-history — both desktop consumers) |
...
| [32-per-guest-value-producer.md](32-per-guest-value-producer.md) | R5 | arch-parameterize `dataflow_emu.c` (arm64 first) | 3 | independent axis; demand-gated | ◐ 2/3 (T1 `df_guest` vtable, byte-identical x86-64; T2 arm64 value fabric + golden `arm64-df-chain`; **arm64 `regstate`/Scrubber + T3 author-run deferred** — need the `emu.c` ring arch-param (R4) / author-run path, see brief status) | *free* (T3 + regstate) |
