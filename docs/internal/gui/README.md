...
> | [53-3d-catalog-build-roadmap.md](53-3d-catalog-build-roadmap.md) | the 3D **depiction** axis (which quantities the scene draws) — the 26-graph catalog cut into phases. **All six briefs (54–59) landed** 2026-08-02; the roadmap stays for its §7 statement of what an uncut Phase 4/5 would contain. |
> | [55-scene-render-quality.md](55-scene-render-quality.md) | ✅ **6/6 landed** 2026-08-02 (T2 halos + T6 step 1 quad expansion closed the last two). Residual: **T6 step 3**, the GLSL-130-on-Apple-core-profile question — *verify-first on Darwin*, and this tree has no macOS desktop lane, so it is a platform gate, not unbuilt work. |
> | [57-causal-layers.md](57-causal-layers.md) | ✅ **5/5 landed** 2026-08-02; doc complete, stays here only until the next archive sweep. Open follow-up: the four layers are **drawn, not picked** (deep-link targets carried as data, no pick band). |
> | [58-memory-data-cell-family.md](58-memory-data-cell-family.md) | ✅ **6/6 landed** 2026-08-02; doc complete, stays here only until the next archive sweep. Open follow-up: the five layers are **drawn, not picked** (no pick band allocated). |
> | [59-standalone-scenes.md](59-standalone-scenes.md) | ✅ **5/5 landed** 2026-08-02; doc complete, stays here only until the next archive sweep. |
...
| [55-scene-render-quality.md](55-scene-render-quality.md) | eye-dome lighting, depth-dependent halos, `fwidth` contour bands + height key, order-independent translucency (WBOIT + dithered fallback), MSAA, portable line width + the GLSL-version question | 6 | none | ✅ 6/6 (T2 `a4175bf` + T6 step 1 `4d8dde2` closed it — **T6 step 1** replaces every colour-path `glLineWidth(>1.0)` with vertex-shader quad expansion, resolving the previous session's stated blocker by *not touching the shared buffer*: the bare-vec3 line VBOs stay exactly as they were and the quads are a **second** buffer, so the pick pass's wide-line draw is untouched call for call, with `Scene::pick_widening` falling to a quad-expanded pick pass only where the driver's `GL_ALIASED_LINE_WIDTH_RANGE` cannot cover the 5–6px click targets. Byte-identity of the pick buffer was **measured, not argued** — an FNV-1a digest over the whole id buffer across all four bands at three radii read `de7e5750c73fcbc9` at the merge base and after both tasks. **T2** found the brief's own step 1 does not work: on a tessellated polyline a nearer segment's halo wins the depth test against a *neighbouring* segment's core and eats its own line (measured: a 16-segment arc lost 78% of itself), and no depth offset rescues it — so the halo is the **outer ring of each line's own quad**, one primitive with no depth relationship to get wrong. A statistical line's ring inherits the line's own IGN stipple mask by construction, so gaps stay gaps. Earlier: T1 EDL `dec808a`; T3 contours `acbf5ae`; T4 dithered translucency `1ebdb69` + its step-5 HUD mode statement, which was credited to that commit but had never actually landed until `translucency_mode_note()` did; T5 MSAA `dec808a`; T6 step 2 only `ada0cb3` — step 1 quad-expansion and T2 halos landed 2026-08-02 — see below) | — |
...
| [57-causal-layers.md](57-causal-layers.md) | L5, L6, L11, L14: one step→place resolver, kernel crossing spurs, taint isochrone, blame convergence forest, dominant-path ridge | 5 | 54 T3 (T2), 54 T5 (T3); 56 T1 (landed) | ✅ 5/5 (T1 `space/stepplace.{h,cpp}` **adopts** 50's `StepAddrResolver` rather than rebuilding it — every rung, the anchor caching and every refusal string come from `locate.cpp` verbatim, T1 adding only u/v, the region and a miss count that counts **distinct steps**, `a106aa1`; T2 kernel crossing spurs in their own TU, out and return meeting at **one** rail point so `rail_span()==0` by construction, `f531b64`; T3 the taint isochrone, exact-only **by type** — the signature takes a `DataflowStream`, so a `SurveyEdge` has nowhere to enter — with "not reached" vs "not recorded" as a **positive** mark, `a76b0f0`; T4 the blame convergence forest, weight keyed by **step** not cell, because summing two steps in one cell would manufacture a convergence no step has, `d2a9f53`; T5 the dominant-path ridge, refusing the greedy block-attribution rule the 2026-07-17 note records shipping twice **plus a second instance that note does not name**, and never blending survey with exact — two builders, two inputs, two types, two GL buffers, `a910aed`) | — |
| [58-memory-data-cell-family.md](58-memory-data-cell-family.md) | L7–L9, L12, L13: data-rung HUD contract, read/write twin relief, working-set tide, observed-lifetime pillars, data-access worldline ribbon, residency sediment columns | 6 | **54 T1 + T2**; 55, 56 T1 (landed) | ✅ 6/6 (T1 `TerrainModel::mem_accesses`/`mem_dropped` — the `mem` scan's `continue` was silent — and `placement_chips` takes over the whole data rung, retiring an inline `chip()` call in `draw_scene_hud` no test could reach, `486520d`; T2 `DataCell::cum_dir`, a prefix OR of **honest** direction bits, because presence was derivable from nothing that existed — `cum_rw` folds an unknown token into `TF_READ` and `cum_read_size > 0` loses a zero-byte access — and an unobserved direction emits **no vertex**, so the absence is geometric, `2fb57bb`; T3 the working-set tide, a windowed delta over the existing prefix sums proved equal to a brute-force sum for every `(t,W)` in a 4×4 grid, `7e15fe2`; T4 observed-lifetime pillars, open-topped (torn tail) ones in their own batch/colour/count, `5388584`; T5 the data-access ribbon with a checkable gap rule — both endpoints placed **and** both steps inside the covered domain, `a08a890`; T6 residency sediment columns on the **existing** degrade path (`kScrubCellBudget` hoisted to a shared `kSceneCellBudget`), halving **bands** never cells, `2a0d02c`) | — |
| [59-standalone-scenes.md](59-standalone-scenes.md) | S1–S4: a scene host that is not the address plane, then divergence worldline, invocation stack, module excursion ribbon, SIMD lane prism | 5 | 54 T6 (T3), 54 T3 (T4); 48, 47 T3 | ✅ 5/5 (T1 `scene3d/scene_kind.h` — the `SceneKind` discriminant and the axis contract, where `scene_axes()` is a switch with **no `default:`** over a complete enum so a kind that forgets to declare its axes fails to *compile*, and each kind must state what its vertical axis is **not**; `Plane` pinned at 0 keeps every existing `SceneFrame` field's meaning and the plane frame byte-identical; `scene3d/glcommon.{h,cpp}` factors out the genuinely shared GL machinery and `scene3d/standalone_gl.{h,cpp}` is the non-plane renderer, `ed8ce64`; T2 divergence worldline `a393ec7`; T3 invocation stack `7a5dcc8`; T4 module excursion ribbon `8f0506e`; T5 SIMD lane prism `ea1db1c`. **Both co-owned mechanisms adopted, not forked**: 47 T3's `PickBands` gains `kind`+`nelem` as an outer band, and `decode_pick` now **refuses** a foreign-kind id rather than aliasing it onto a cell; the camera frames each kind through 48 T1's movable `Camera::target`) | — |

67 tasks across the three families — **all 67 landed** (2026-08-02): 7 in
representation via 44, all 26 in instrument via 47/48/49/50/51/52, and all 34
in depiction via 54–59. **Every implementation brief cut from all three 3D
roadmaps is now complete.** What remains in this family is not unbuilt tasks
but uncut scope: [43](43-faithful-city-roadmap.md)'s city Phases B–E, and
[53](53-3d-catalog-build-roadmap.md)§7's deliberately-uncut Phase 4/5 (seven
large scenes). Two residuals are recorded rather than open — 55 T6 step 3 is
a Darwin verify-first with no macOS lane in this tree, and 57/58's layers are
drawn but not picked (their deep-link targets are carried as data and tested;
only the pick bands are missing, and 59 has since landed the outer kind band
that makes allocating them cheap). Sequencing across
...
address-first step→place resolver**: 50 (landed — `space/locate.h`'s
`scene_locate_off`/`scene_locate_step`), 57 T1 adopts it. **The movable camera
target**: 48, 59 T5's per-scene framing. In each case whichever lands first owns the mechanism
...
arcs/columns) — so 57 and 58's own "56 T1" prerequisite was satisfied, and
**57, 58 and 59 all landed the same day**, closing the family. They were
implemented in parallel in isolated worktrees, which is worth recording
because the merge — not the implementation — is where the real defects were:
57 and 58 both named a `SceneLayers` member in `test_layers.cpp`'s exhaustive
`members[]` array, and keeping both sides duplicated `mispred` (caught only by
56 T1's own row-count assertion, since the per-member "exactly one row" loop
passed for it twice); 58 and 59 both claimed the field name `ribbon` in
`SceneFrame`/`SceneView` for two unrelated things, so the second assignment
silently overwrote the first (59's is now `module_ribbon`); and 55 and 59 both
wanted GL attribute slot 2, which is precisely the hazard `glcommon.h` was
written to prevent. The catalog's Phase 4/5 —
