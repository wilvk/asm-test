...
| [ptrace-blockstep-tracer-correctness.md](ptrace-blockstep-tracer-correctness.md) — int3 si_code, rep-prefix, SP-aware step-over, IBS pre-cover | 8 | ◐ 6/8 (T1 [all 3 drivers], T2, T3, T4, T5, T6 landed & verified: `make docker-hwtrace` 436/436 (+4 for T4's re-entrant call-out differential — confirmed red, with the correct-invocation result flipping to the inner one, when the depth check is reverted), `docker-hwtrace-jit-dotnet-bcl`/`-jit-java` green on real CoreCLR/HotSpot call-out step-overs, `make docker-docs` clean; block-step legs self-skip on this BTF-masked host, consistent with prior legs; T7 wip; **UNCLAIMED**: T8 precover policy bit [needs T7]) | — |
