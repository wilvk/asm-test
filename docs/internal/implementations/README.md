...
| [dataflow-bindings-slice-codeimage.md](dataflow-bindings-slice-codeimage.md) — def-use/slice surface + code-image arg across bindings | 4 | ◐ 1/4 (T1 done: `asmtest_slice_forward_seed`/`_backward_seed` exported from `libasmtest_dataflow`, byte-for-byte parity with the by-value slicers proven over every step index of a known graph; `make dataflow-test` 32/32 pure suite, `nm -D` confirms both symbols, `dataflow-python-test` 17/17 and `dataflow-node-test` 44/44 unaffected — the by-value path Python/C++/Node use is untouched; T2 wip: wraps the def-use + slice surface in the seven remaining bindings; **UNCLAIMED**: T3 tests it [needs T2], T4 wraps the code-image recorder [independent, no deps]) | — |
