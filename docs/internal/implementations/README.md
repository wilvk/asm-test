...
| Document | Tasks | Status | Depends on |
|---|---|---|---|
| [asmspy-cli-enhancements.md](asmspy-cli-enhancements.md) — TUI hot-edge drill-in, syscall-arg content decode, coverage gaps | 9 | ◐ 1/9 (T2 wip) | — |
...
| Document | Tasks | Status | Depends on |
|---|---|---|---|
| [distribution-packaging.md](distribution-packaging.md) — language-registry go-live + system package-manager coverage | 13 | ◐ 4/13 (T7 `make package-source` reproducible tarball + release asset; T8 Homebrew, T9 Debian `libasmtest-dev`, T10 AUR `PKGBUILD` — `make docker-syspkg-{brew,deb,aur}` all green (build + lint [brew audit/style, lintian, namcap] + install + pkg-config consumer). Hermetic on the local `package-source` tarball because the v1.1.0 release asset the manifests pin is T3-gated (absent in-tree). MIT-only per-manager manifests under `packaging/`; T11 wip.) | — |
