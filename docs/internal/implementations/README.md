...
| [pin-probe-mode-capture.md](pin-probe-mode-capture.md) — Pin probe-mode argument/return capture | 7 | ◐ 2/7 (T1 substrate reuse; T2 `include/asmtest_valtrace_shm.h` cross-process value channel + `examples/pin_probe_victim.c` capture fixture — header compiles `-std=c11 -Wall -Werror` AND under PinCRT, `capref`/`tiny`/attach-loop fixture modes all exit 0) | **pin-xed-trace-tier** |
