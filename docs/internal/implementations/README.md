...
| [ptrace-blockstep-tracer-correctness.md](ptrace-blockstep-tracer-correctness.md) — int3 si_code, rep-prefix, SP-aware step-over, IBS pre-cover | 8 | ◐ 3/8 (T1 region+attached, T3, T6 landed & verified locally; **UNCLAIMED**: T1 windowed leg, T2 per-insn SIGTRAP forwarding, T4 SP-aware step-over [needs T2], T5 descent cleanups, T7 IBS pre-cover [needs T1], T8 precover policy bit [needs T7]) | — |
...
| Document | Tasks | Status | Depends on |
|---|---|---|---|
| [inproc-btf-block-step.md](inproc-btf-block-step.md) — W3: in-process BTF branch-granular single-step | 6 | ◐ 1/6 (T2 done, verified live on this Zen 2 host — BTF is genuinely functional, not just gated) | — |
