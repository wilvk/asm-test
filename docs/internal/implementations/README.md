...
| [libfuzzer-afl-shim.md](libfuzzer-afl-shim.md) — libFuzzer/AFL harness shim (demand-gated) | 5 | ◐ 2/5 (T1 emu_cover_hits seam + T2 libFuzzer harness landed; verified in a clang-18 fuzz image: baseline CLASSIFY3 -runs=50000 exit 0 cov climbs, crash variant writes a crash-* artifact via external `__sanitizer_cov_8bit_counters_init`/`_pcs_init`. doc correction: clang-18 libFuzzer requires the parallel PC table, not "symbolization only". Dockerfile.fuzz/docker-fuzz land in T4) | — |
