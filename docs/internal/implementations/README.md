...
| [pin-probe-mode-capture.md](pin-probe-mode-capture.md) — Pin probe-mode argument/return capture | 7 | ◐ 5/7 (T5 probe-refusal detection — per-target skip with reason: `capref` captures (skip=0), `tiny` (1-byte `ret`, RTN_Size 1 < 14) -> `AV_SKIP_TOO_SHORT`, absent symbol -> `AV_SKIP_NOT_FOUND`, all done=1. **Doc/Pin correction:** Pin 4.2 `RTN_IsSafeForProbedInsertion` is permissive (reports even a 1-byte routine safe + relocates), so the reliable signal is `RTN_Size` vs the 14-byte probe floor — the doc's own 'synthesize from RTN_Size' path; `RTN_InsertCallProbed` returns BOOL here (not VOID) so the post-check is authoritative) | **pin-xed-trace-tier** |
