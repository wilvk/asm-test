...
| [dataflow-f4-object-identity.md](dataflow-f4-object-identity.md) — real object identity via GCBulkType/Node/Edge | 6 | ◐ 2/6 (T1: `asmtest_objid_*` + `test_dataflow_objid` 27/27; T2: post-capture `GCCANON_MOVE_POST` feed, `docker-gccanon-attach` 37/37 with `post_moves` 647>0 every phase; T3 wip) | — |
...
| [riscv-native-tier.md](riscv-native-tier.md) — native RISC-V (rv64) host tier | 7 | ◐ 4/7 (T3/T6/T7 remain) | — |
