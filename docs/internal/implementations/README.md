...
| Document | Tasks | Status | Depends on |
|---|---|---|---|
| [intel-pt-whole-window-substrate.md](intel-pt-whole-window-substrate.md) — PT whole-window capture substrate, STRONG ladder, inline ctor | 5 | ◐ 4/5 (T5 wip) | — |
...
| Document | Tasks | Status | Depends on |
|---|---|---|---|
| [distribution-packaging.md](distribution-packaging.md) — language-registry go-live + system package-manager coverage | 13 | ◐ 7/13 (system-package stream COMPLETE — T7 `make package-source` + release asset; T8-T12 five specs under `packaging/` (Homebrew, Debian `libasmtest-dev`, AUR, vcpkg overlay port, Conan 2); T13 aggregate `make docker-syspkg` + additive `syspkg` CI job (matrix ×5) + user docs (installation.md) + submission runbook (releasing.md). `make docker-syspkg` GREEN (all five build+lint+install+consumer), `make docker-docs` clean, registry names verified free. Registry go-live **T1-T6 credential-gated** (PyPI/npm/crates/RubyGems/NuGet/Maven/LuaRocks accounts+tokens + tag push this host lacks) — T1-T6 gated scaffolding wip.) | — |
