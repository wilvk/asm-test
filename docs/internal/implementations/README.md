...
| [benchmarks-ci-followups.md](benchmarks-ci-followups.md) — windows/macOS-Intel legs, nightly auto-commit, BM_MODEL_COST | 6 | ◐ 1/6 (T5 landed & verified: `asmtest_disas_class` {OTHER/MEM/BRANCH/MULDIV} classifier, `make emu-test` 55/55 native + no-Capstone `docker-test` degrade path both green; T2/T6 wip) | — |
