...
| Document | Tasks | Status | Depends on |
|---|---|---|---|
| [dataflow-producer-correctness.md](dataflow-producer-correctness.md) — gap barrier, sub-register aliases, undefined flags, F2 record-and-inject | 8 | ◐ 3/8 (T1, T2, T4 done & live-verified; T2 on the Zen 5 box — `make docker-dataflow-attach` 415/415, `dataflow-blockstep-test` 119/119, `docker-gccanon-attach` 37/37, all 0 skips; adversarially reviewed, one confirmed pre-existing/shared risk-set-cap limitation documented at `dfp_risk_flag`, not a T2 regression) | — |
