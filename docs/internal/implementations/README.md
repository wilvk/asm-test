...
| [pin-probe-mode-capture.md](pin-probe-mode-capture.md) — Pin probe-mode argument/return capture | 7 | ◐ 6/7 (T6 cross-check + lane: `examples/pin_probe_validator.c` + `make pin-probe-test` wired into `docker-pintool` (--cap-add=SYS_PTRACE). **`make docker-pintool` GREEN in-container**: pin-xed PIN-2 11/11 + apx 2/2 AND pin-probe-test 21/21 — capture 12/12 with the LIVE independent ptrace producer agreeing (`ptrace rdi=11 rsi=22 rax=133 == pin`), badptr 5/5 (invalid ptr refused not faulted), skip 4/4 (tiny -> TOO_SHORT)) | **pin-xed-trace-tier** |
