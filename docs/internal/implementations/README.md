...
| [pin-probe-mode-capture.md](pin-probe-mode-capture.md) — Pin probe-mode argument/return capture | 7 | ◐ 3/7 (T3 `pintool/probe_capture.cpp` probe-mode entry/exit capture — builds under PinCRT; a live `pin -probe -t probe_capture.so -func capref` fills the channel: RDI=11 RSI=22 XMM0=3.5 RAX=133 RFLAGS=0x202, done=1 skip=0. FP arg read via IARG_FUNCARG_ENTRYPOINT_VALUE — probe-mode IARG_CONTEXT carries no SSE state) | **pin-xed-trace-tier** |
...
| Document | Tasks | Status | Depends on |
|---|---|---|---|
| [aarch64-sve-capture.md](aarch64-sve-capture.md) — AArch64 SVE wide-vector capture | 8 | ◐ 6/8 (T7 wip) | — |
...
| [libfuzzer-afl-shim.md](libfuzzer-afl-shim.md) — libFuzzer/AFL harness shim (demand-gated) | 5 | ◐ 2/5 (T1 emu_cover_hits seam + T2 libFuzzer harness landed; verified in a clang-18 fuzz image: baseline CLASSIFY3 -runs=50000 exit 0 cov climbs, crash variant writes a crash-* artifact via external `__sanitizer_cov_8bit_counters_init`/`_pcs_init`. doc correction: clang-18 libFuzzer requires the parallel PC table, not "symbolization only". Dockerfile.fuzz/docker-fuzz land in T4; T3 wip) | — |
