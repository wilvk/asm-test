...
| [pin-probe-mode-capture.md](pin-probe-mode-capture.md) — Pin probe-mode argument/return capture | 7 | ◐ 1/7 (T1 substrate reuse verified: `scripts/fetch-pin.sh` + `Dockerfile.pintool` + `docker-pintool` all present at the single pinned `PIN_VERSION=4.2-99776-g21d818fa2`, no second Pin version) | **pin-xed-trace-tier** |
...
| Document | Tasks | Status | Depends on |
|---|---|---|---|
| [aarch64-sve-capture.md](aarch64-sve-capture.md) — AArch64 SVE wide-vector capture | 8 | ◐ 2/8 (T3 wip) | — |
...
| [libfuzzer-afl-shim.md](libfuzzer-afl-shim.md) — libFuzzer/AFL harness shim (demand-gated) | 5 | ◐ 1/5 (T1 landed: emu_cover_hits seam, `make emu-test` 56/56 x86-64 + arm64 lane 41/56 with test 27 live; T2 wip) | — |
