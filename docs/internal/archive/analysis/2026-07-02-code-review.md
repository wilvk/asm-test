...
| #19 call-out not call-depth aware | ✅ Fixed | batch-d header correction, then the depth-aware `run_until_sp` step-over — [ptrace-blockstep-tracer-correctness.md](../../archive/implementations/ptrace-blockstep-tracer-correctness.md) T4 |
...
**Triage (2026-07-17).** All nine findings below were actioned by
[code-review-plausible-triage.md](../../archive/implementations/code-review-plausible-triage.md).
Seven were still-live bugs, each fixed with a regression test; two had
already been fixed independently before this triage swept the list. The
findings themselves are left unedited below as the historical record — this
table is additive.

| Finding | Disposition |
|---|---|
| `src/asmtest.c:1242` — `run_parallel`'s non-EINTR `poll()` failure reports never-run tests as PASSED | Fixed, [code-review-plausible-triage.md T1](../../archive/implementations/code-review-plausible-triage.md#t1--make-run_parallel-survive-a-non-eintr-poll-failure-without-reporting-unrun-tests-as-passed--s-depends-on-none): degrade to blocking reaps (`fix(runner): survive non-EINTR poll() failure without false-green results (code-review-plausible-triage T1)`, commit `6dcf8ba`) |
| `src/glob_match.c:25` — `glob_match` diverges from `fnmatch(flags=0)` on unterminated `[`, in-class escapes, trailing `\` | Fixed, [T2](../../archive/implementations/code-review-plausible-triage.md#t2--bring-glob_match-to-fnmatchflags0-parity-unterminated--in-class-escapes-trailing-backslash--m-depends-on-none): `class_end` helper + escape-aware `match_class` + trailing-backslash rejection, proven against the host's real `fnmatch` by a new differential harness (`fix(glob): bring glob_match to fnmatch(flags=0) parity (code-review-plausible-triage T2)`, commit `57eb8d0`) |
| `src/asmtest.c:482` — `asmtest_guarded_alloc` size arithmetic overflows near `SIZE_MAX`, returning a guard-page pointer instead of NULL | Fixed, [T3](../../archive/implementations/code-review-plausible-triage.md#t3--reject-guard-page-allocations-that-overflow-the-page-rounding-posix-and-win32-twins--s-depends-on-none): `round_up_page` returns 0 on overflow (POSIX and the Win32 twin) (`fix(alloc): reject guard-page allocations that overflow page rounding (code-review-plausible-triage T3)`, commit `edf0c14`) |
| `src/fuzz.c:54` — corpus "nudge" mutation signed-overflows at range extremes (UB) | Fixed, [T4](../../archive/implementations/code-review-plausible-triage.md#t4--remove-the-signed-overflow-ub-in-the-fuzz-corpus-nudge--s-depends-on-none): unsigned-arithmetic idiom mirroring `asmtest_rng_range`, proven against `make docker-sanitize`'s UBSan lane (`fix(fuzz): remove signed-overflow UB in the corpus nudge mutation (code-review-plausible-triage T4)`, commit `e494f17`) |
| `src/drtrace_app.c:161` — `asmtest_dr_available()` not in lock-step with `dr_lib_path()`'s bare-soname fallback | Independently fixed before triage: `src/drtrace_app.c` 69–75 now documents the `dr_lib_path`/`asmtest_dr_available` bare-soname divergence explicitly. No task in code-review-plausible-triage.md; see its "What already exists" / "Out of scope" sections |
| `src/drtrace_client.c:209` — DR client begin/end stack desynchronizes past `MAX_DEPTH` | Independently fixed before triage: `src/drtrace_client.c` 208–221 tracks begin/end depth unconditionally with a `truncated` flag past `MAX_DEPTH`. No task in code-review-plausible-triage.md; see its "What already exists" / "Out of scope" sections |
| `bindings/zig/src/conformance.zig:387` — Zig conformance tests pass 2-element vector-arg arrays where the trampoline reads 8 (192/384-byte stack over-read) | Fixed, [T5](../../archive/implementations/code-review-plausible-triage.md#t5--give-the-zig-vec256vec512-conformance-tests-full-8-slot-vargs-arrays--s-depends-on-none): full 8-element zero-initialized arrays, mirroring the C `ASM_VCALL256_2`/`512_2` macros (`fix(zig): give the vec256/vec512 conformance tests full 8-slot vargs arrays (code-review-plausible-triage T5)`, commit `c61a70e`) |
| `src/platform_win32.c:369` — `rt_veh_cb`'s arming is process-global; a non-test-thread fault is longjmp'd onto the test thread's stack | Fixed, [T6](../../archive/implementations/code-review-plausible-triage.md#t6--scope-the-win32---no-fork-veh-crash-handler-to-the-test-thread--m-depends-on-none): a recorded `rt_test_tid` gate scopes redirection to the armed test thread only, mirroring `asmtest_win32_guard`'s `__thread` scoping (`fix(win64): scope the --no-fork VEH crash handler to the test thread (code-review-plausible-triage T6)`, commit `5381889`) |
| `docs/emulator.md:38` — the emulator guide claims `--emu` installs "libunicorn (and only that)" | Fixed, [T7](../../archive/implementations/code-review-plausible-triage.md#t7--fix-the-emulator-guides---emu-install-claim--s-depends-on-none): corrected to unicorn + pkg-config (Capstone source-built), mirroring `installation.md`'s existing phrasing (`docs(emulator): fix the --emu install claim (code-review-plausible-triage T7)`, commit `dd269b3`) |
