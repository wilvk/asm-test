...
> directory's [README.md](../../gui/README.md); siblings are linked where referenced.
...
Everything named `asmdesk::*`, every `desktop/*` file, `mk/desktop.mk`, `Dockerfile.desktop`, the two fetch scripts, and targets `desktop`/`desktop-render`/`desktop-test`/`docker-desktop` are **new** (introduced here). `make asmtrace-golden`, `tests/golden-asmtrace/`, and [asmtrace-schema.md](../../gui/asmtrace-schema.md) are **new, owned by 01**.
...
3. Compute both SHA-256es locally (`curl -fsSL <url> | sha256sum`) and append two rows **by hand** (refresh-script caution above), with a comment citing the self-hashed precedent ([third-party-digests.txt:23](../../../../scripts/third-party-digests.txt#L23)):
   ```
   # imgui + nlohmann-json: the desktop GUI (docs/internal/archive/gui/03-desktop-shell.md).
...
# (docs/internal/archive/gui/03-desktop-shell.md). Included BEFORE mk/bindings.mk, so
...
> [asmtrace-schema.md](../../gui/asmtrace-schema.md) and `tests/golden-asmtrace/`) instead
...
**Steps.**
1. Create `desktop/src/doc/recording.h`/`.cpp` and `workspace.h`/`.cpp` (Code). Field names follow [asmtrace-schema.md](../../gui/asmtrace-schema.md) (owned by 01) — 01 wins on names, this doc wins on loader behavior.
