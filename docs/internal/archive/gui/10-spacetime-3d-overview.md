...
> decisions D1–D11 live in this directory's [README](../../gui/README.md). Siblings:
...
>
> **Extended 2026-07-31** by [43-faithful-city-roadmap.md](../../gui/43-faithful-city-roadmap.md)
...
### T7 — Golden scenes, gated GL lane, docs, changelog  (M, depends on: T1–T6)  ✅ DONE

> Landed. **Three** golden scenes, all rendered end-to-end by
> `desktop/test/test_scene_fbo.cpp` under surfaceless EGL + software Mesa. Two are
...
> holding 6 of 13 steps → `truncated` + `drops.lost` → `TF_TORN` on every
> populated cell). Byte-stable across regenerations on the host and in the
> authoritative docker-cli image; no existing golden's bytes changed. The rich
> `mem` scene is hand-authored at `tests/golden-asmtrace/scenes/mem-rich-synthetic.asmtrace`,
> marked **schema-unfrozen** (`backend`/`trust` say so in the data as well as in
> the note) and **inert-but-present**: its accesses lie outside every code region,
> so the coarse-rung projection gains no data cell and the scene renders
> pixel-identically to its coarse twin. The `TF_STAT` half REUSES the committed
> `desktop/test/fixtures/obs-survey-ibs.asmtrace` rather than adding a second
> survey fixture, and isolation is asserted at the pixel level (turning the exact
> layer off changes no pixel of a survey-only scene). Step 2 needed nothing:
...
5. Update this directory's [README](../../gui/README.md) status row and the plan pointer
