...
> [AMD tracing plan](amd-tracing-plan.md) (the tiers themselves) and the
...
> **no untested hardware code** — which is why P4 ships documentation only and
> its code half stays gated on silicon this project does not have.
...
|---|---|---|---|---|
| **P1** | Un-stale [amd-hardware-validation.md](../amd-hardware-validation.md) — the only AMD gate; 3 of its checks are now wrong, one inverts a regression signal | **high** | S | no |
| **P2** | `branchsnap` depth check counts a synthetic edge → spurious `truncated` at `use == 15` → a real re-execution | med | S | no |
| **P3** | `IBS_MAX_RECORD` is pre-callchain → the loss heuristic is ~10× short → **silent** ring loss with `lost==0 && throttled==0` | med-high | S | no |
| **P4** | Zen 3 BRS: three docs assert a working path the kernel proves cannot open (one is a Phase marked *landed* specifying the exact missing arm) | med | S | **docs no / code yes** |
| **P5** | `asmtest_amd_freeze_available()` is dead; one site prints a **flatly false** string to a human; 3 plan sections marked *landed* assert the opposite of shipped code | low | S | no |
| **P6** | `asmtest_ibs.h` is not installed, though the guide tells users to `#include` it | med | XS | no |
| **P7** | Record the kernel append-order dependency the IBS bit-5 caps gate silently relies on | low | S | no |
| **P8** | `sample_period` / `period_jitter` are set **nowhere in the tree, not even in tests** — the rounding, clamp and ABI guard have never executed | low-med | S | no |
| **P9** | Self-hosted Zen runner — closes the class of gap that hid the `call_auto` bug | **high** | L | infra |

---

## P1 — Repair the AMD validation checklist *(planned)*
...
3. **`:121-122`** gates on `ibs_probe` printing **AVAILABLE** and on `test_ibs`
   passing. [asmspy-plan.md:131](asmspy-plan.md), measured 2026-07-17, records
...
## P2 — `branchsnap`: gate the depth check on `use`, not `n_dec` *(planned)*
...
## P3 — `IBS_MAX_RECORD`: size it for the callchain worst case *(planned)*
...
## P4 — Zen 3 BRS: make the docs true; leave the code gated *(planned)*
...
- [amd-tracing-plan.md:173-176](amd-tracing-plan.md) — **Phase 0, marked
...
- Improvement Phase 6 ([:1314+](amd-tracing-plan.md)) is scoped as a *throughput*
...
- Phase 6 is rescoped: it is not a throughput optimisation but the **only way BRS
  ...
  ([:1332-1336](amd-tracing-plan.md)), so BRS must be a distinct Tier-A mode —
...
## P5 — Retire the freeze probe or wire it; kill the false printf *(planned)*
...
- [amd_backend.h:32-34](../../../src/amd_backend.h) is stale *by implication*
  ...
  same files.
- [amd-tracing-plan.md:560-570](amd-tracing-plan.md) (marked **landed**), `:996`,
...
## P6 — Install `asmtest_ibs.h` *(planned)*
...
## P7 — Record the caps append-order dependency *(planned)*
...
## P8 — Exercise `sample_period` / `period_jitter` at all *(planned)*
...
## P9 — Self-hosted Zen runner *(forward-look)*
