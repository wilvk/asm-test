...
- **Non-blocking drain in snapshot end() — F15** (`src/branchsnap.c:228-230`). `ring_buffer__poll(rb, 200)` does a 200 ms `epoll_wait` before draining; on the no-hit (faithful-truncation) path nothing ever wakes epoll, so end() stalls the full 200 ms and drains nothing. Records are already synchronously `bpf_ringbuf_submit`ted in the tracee's context during `run_fn`. **Fix:** `ring_buffer__consume(g_bsnap.rb)`. Zen 4+ only.
