/*
 * disasm.c — instruction-level disassembly for emulator diagnostics, backed by
 * Capstone (the disassembler counterpart to the Keystone assembler in
 * assemble.c). Turns the byte offsets the emulator records for faults, traces,
 * and coverage into readable instructions: `uncovered: 0x2f` becomes
 ...
 * emu.o) verbatim for the no-Capstone / no-code path, so the offset-only output
...
/* Map an emu_arch_t to Capstone's (arch, mode). Returns false for a guest this
 * Capstone build can't disassemble — notably RISC-V, which needs Capstone >= 5
 * (the CS_ARCH_RISCV enumerator does not exist before then, so it is guarded by
 * the API-version macro rather than referenced unconditionally). */
static bool cs_target(emu_arch_t arch, cs_arch *a, cs_mode *m) {
    switch (arch) {
    case EMU_ARCH_X86_64:
        *a = CS_ARCH_X86;
        *m = CS_MODE_64;
        return true;
    case EMU_ARCH_ARM64:
 ...
 * *out_n receives the count. NULL on empty/oom. Mirrors emu.c's sorted_blocks so
 * the disassembling reports list blocks in the same order as the plain ones. */
...
static void print_block_line(FILE *out, emu_arch_t arch, const uint8_t *code,
                             size_t code_len, uint64_t off) {
    char text[128];
    emu_disas(arch, code, code_len, EMU_CODE_BASE, off, text, sizeof text);
    if (text[0] != '\0')
...
void emu_trace_disasm(const emu_trace_t *t, emu_arch_t arch,
    ...
    if (t->insns == NULL)
        return;
    bool disasm = emu_disas_available() && code != NULL;
    for (size_t i = 0; i < t->insns_len; i++)
        if (disasm)
            print_block_line(out, arch, code, code_len, t->insns[i]);
        else
...
void emu_trace_report_disasm(const emu_trace_t *t, emu_arch_t arch,
                             const uint8_t *code, size_t code_len, FILE *out) {
    if (!emu_disas_available() || code == NULL) {
    ...
    for (size_t i = 0; i < n; i++)
        print_block_line(out, arch, code, code_len, s[i]);
    free(s);
}
    ...
    if (out != NULL) {
        ...
        if (nmiss > 0) {
            ...
            for (size_t i = 0; i < nmiss; i++)
                print_block_line(out, arch, code, code_len, miss[i]);
        }
    }
    free(miss);
    return nmiss;
...
void emu_fault_describe(const emu_result_t *r, emu_arch_t arch,
    ...
    char text[128];
    emu_disas(arch, code, code_len, base_addr, off, text, sizeof text);
    if (text[0] != '\0')
