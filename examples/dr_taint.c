...
#include "taint_fixtures.h" /* shared seed/sink fixtures + region metadata */
...
/* The fixture byte arrays + region metadata (taint_chain / taint_sink_chain /
 * taint_heapstore / taint_callarg / taint_memlen; SINK_OFF/SINK_DEPTH/CALLARG_OFF/
 * MEMLEN_OFF/MEMLEN_DEPTH) moved VERBATIM to examples/taint_fixtures.h so the Pin/libdft
 * oracle runs the byte-identical inputs (included above). This lane is the proof no
