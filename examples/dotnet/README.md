...
| [amdhot/](amdhot/) | `AsmTrace.WindowHot(() => {…})` | the **statistical AMD-LBR whole-window survey** — AMD LBR samples the branch stack out of band while the managed block runs at **native speed**, bucketing sampled branch-target endpoints into a **hot-method histogram** (`IsStatistical`, `Methods[].Count` = sample weight). Crash-proof (no `EFLAGS.TF`/SIGTRAP) and near-native, the honest AMD whole-window shape (exact is a hardware dead end). Needs Zen 3+/LBR + `CAP_PERFMON` → self-skips in the plain lane |
