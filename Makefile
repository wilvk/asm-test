...
help:
	...
	@echo ''
	@echo 'asmspy — out-of-band process watcher (Linux x86-64/AArch64):'
	@echo '  cli             build build/asmspy (no args = TUI; --help for the headless modes)'
	@echo '  cli-smoke       headless end-to-end smoke over the example victims (make docker-cli)'
	@echo '  docker-cli-ibs  the same smoke with perf access, so --sample really runs'
	@echo '                  Every headless mode takes --record=<f> to write a .asmtrace NDJSON'
	...
	@echo '  win64-bench-record  persist this Windows box report into benchmarks/boxes/ (real Win)'
	@echo ''
